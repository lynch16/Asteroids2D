...
func _process(_delta: float) -> void:
	...
	if (viewport_rect.has_point(mouse_down_position)):
...
func _track_verts(tile_center: Vector2) -> void:
	...
	if (tile_index == proc_gen_utils.ALL_CORNERS):
		if (
			(tile_center.x + proc_gen_utils.TILE_SIZE > viewport_rect.size.x) || \
			(tile_center.y + proc_gen_utils.TILE_SIZE > viewport_rect.size.y) || \
			(tile_center.x - proc_gen_utils.TILE_SIZE < 0) || \
			(tile_center.y - proc_gen_utils.TILE_SIZE < 0)
		):
			print("INCLUDE");
			# TODO: Edges not yet included automatically. The tile walker never moves into this block
		print("ALL_CORNERS");
	elif (tile_index == proc_gen_utils.NO_CORNERS):
		print("NO CORNERS");
	else:
		var next_tile_dirs := MSMeshes.NEXT_VERTEX_ARRAYS[tile_index];
		for i in next_tile_dirs.size():
			var next_tile := next_tile_dirs[i] * proc_gen_utils.TILE_SIZE + tile_center;
...
func _handle_mouse_right_click() -> void:
	var mouse_down_position := get_viewport().get_mouse_position();
	var tile_center := proc_gen_utils._get_position_tile_center_coord(mouse_down_position);
...
func _handle_mouse_click() -> void:
	var mouse_down_position := get_viewport().get_mouse_position();
	var corner := proc_gen_utils._get_position_tile_corner_coord(mouse_down_position);
...
func _sample_viewport() -> void:
	...
	proc_gen_utils._for_each_tile(_save_corner_samples);
	_generate_mesh();
	
func _save_corner_samples(center: Vector2) -> void:
	for i: int in proc_gen_utils.CORNERS.size():
		var corner := center + proc_gen_utils.CORNERS[i] * float(proc_gen_utils.HALF_TILE_SIZE);
		if (!corner_sample_tracker.has(corner)):
...
func _generate_mesh() -> void:
	...
	proc_gen_utils._for_each_tile(_generate_tile_from_corners);
...
func _add_tile(center: Vector2, tile_index: int) -> void:
	...
	var texture: CompressedTexture2D = mesh_instance.texture;
...
func _calculate_weighted_vertex(center: Vector2, normalized_vertex: Vector2) -> Vector2:
	...
	var vertex := normalized_vertex * float(proc_gen_utils.HALF_TILE_SIZE) + center;
	#if (is_edge):
